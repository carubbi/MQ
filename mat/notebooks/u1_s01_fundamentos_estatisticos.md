...
1. carregar e inspecionar a estrutura do conjunto de dados Palmer Penguins;
2. identificar fonte, registro, unidade de análise, variáveis e tipos de dados
   reconhecidos pelo `pandas`;
3. formular uma pergunta estatística;
4. distinguir descrição e inferência;
5. delimitar população, amostra, representatividade e limites das conclusões.
...
O notebook não deverá transformar a inspeção computacional em prova de
representatividade. A origem e a forma como os dados foram coletados
permanecerão centrais.

## 4. Preparação conceitual

Antes do código, a futura implementação deverá apresentar:

- uma revisão breve do conjunto sintético de requisições usado nos slides,
  apenas para aplicar os conceitos a outro conjunto de dados;
...
- $\theta$ como parâmetro e $\widehat{\theta}$ como estatística usada para
  estimar o parâmetro;
...
- Origem dos dados: pacote `palmerpenguins` 0.1.6 e projeto Palmer Station LTER.
- Dimensões informadas na documentação: 344 registros e 17 colunas.
...
Selecionar, depois do carregamento, um pequeno conjunto de linhas do próprio
arquivo com identificador, espécie, ilha, data e uma medida corporal.

Esse conjunto não repetirá o contexto dos servidores. Ele deverá aplicar ao
Palmer Penguins as distinções já construídas entre linha, observação, unidade
de análise, variável, valor ausente, descrição e inferência.
...
Essa classificação preparará a leitura de `DataFrame.dtypes` sem confundir o
tipo de dado reconhecido pelo `pandas` com o papel estatístico.
...
- **Conteúdo:** identificação, fonte, contexto, pergunta principal e resposta esperada.
...
- **Produto esperado:** dados armazenados em um `DataFrame`, sem fornecer
  resultados inventados no roteiro.
- **Verificação:** confirmar existência do arquivo e sucesso da leitura.
- **Tempo previsto:** 15 minutos.

### Bloco 4 — Número de linhas, colunas e nomes das variáveis

- **Tipo de célula:** código.
- **Função pedagógica:** inspeção.
- **Ações previstas:** consultar `DataFrame.shape`, `DataFrame.columns` e `DataFrame.head()`.
- **Produtos esperados:** número de linhas e colunas, nomes das colunas e
  primeiras linhas.
- **Pergunta interpretativa:** o que representa uma linha e quais colunas ajudam a justificar essa interpretação?
- **Tempo previsto:** 20 minutos.

### Bloco 5 — Tipos de dados no pandas

- **Tipo de célula:** código.
- **Função pedagógica:** inspeção.
- **Ação prevista:** consultar `DataFrame.dtypes`.
- **Produto esperado:** tipos de dados reconhecidos pelo `pandas` em cada coluna.
- **Pergunta interpretativa:** o tipo de dado reconhecido pelo `pandas` é
  suficiente para classificar estatisticamente cada variável?
- **Tempo previsto:** 20 minutos.
...
- **Conteúdo:** identificação justificada da fonte, dos registros, da unidade
  de análise, de duas variáveis, de suas classificações estatísticas e dos
  tipos de dados reconhecidos pelo `pandas`.
...
- **Conteúdo:** distinção entre descrição e inferência; formulação de uma
  pergunta estatística; declaração da população de interesse, da amostra
  disponível e dos limites das conclusões.
...
- **Conteúdo:** comparação entre a classificação estatística e o tipo de dado
  usado pelo `pandas`; diagnóstico da correspondência entre a pergunta, os
  registros disponíveis e a população de interesse.
- **Tempo previsto:** 20 minutos.

### Bloco 9 — Conclusão justificada

- **Tipo de célula:** Markdown.
- **Função pedagógica:** conclusão e justificativa.
- **Conteúdo:** resposta à pergunta principal, acompanhada de fonte, unidade,
  variáveis, população, limitações dos dados e limites da conclusão.
- **Tempo previsto:** 25 minutos.
...
- classificação estatística e tipo de dado usado pelo `pandas`;
...
Diferenças entre o significado descrito na documentação e o tipo de dado usado
pelo `pandas` deverão ser explicadas, não apenas apontadas.
...
- duas colunas com suas classificações estatísticas e seus tipos de dados no
  `pandas`;
- uma pergunta estatística;
- a população de interesse;
- até onde a conclusão pode ser aplicada e uma limitação dos dados.

Apenas exibir `head()` ou `dtypes` não demonstra compreensão; o estudante
deverá interpretar as informações apresentadas.
...
3. Que generalização da conclusão exigiria inferência?
...
- Executar do início ao fim, sem depender de variáveis criadas fora da
  sequência do notebook.
...
- Separar a classificação estatística dos tipos de dados reconhecidos pelo
  `pandas`.
...
- Comparar a classificação estatística com o tipo de dado usado pelo `pandas`.
